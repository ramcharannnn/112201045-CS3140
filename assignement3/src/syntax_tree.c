...
// Combined function for creating declaration-like statements
stmt_list* newDeclarationStmt(stmt_type type, int ret_type, decl_node* node) {
    ...
    new_stmt->type = type;
    new_stmt->next = NULL;
    new_stmt->tree.decl_list = node; // Assign the declaration node
...
stmt_list* newBegStmt(int ret_type, decl_node* node) {
    ...
    new_stmt->next = NULL;
    new_stmt->tree.decl_list = node; // You might need to adjust this based on your structure
    ...
    while (root != NULL) {
        ...
        switch (root->type) {
            ...
            case COND_STMT:
                ...
                printf("Condition:\n");
                printExpr(root->tree.cond_stmt.condition, indent + 1);
                printf("    ");
                for (int i = 0; i < indent; ++i) {
                    printf("|   ");
                }
                printf("Then Branch:\n");
                printAst(root->tree.cond_stmt.s1, indent + 1);
                if (root->tree.cond_stmt.s2) {
                    ...
                    printf("Else Branch:\n");
                    printAst(root->tree.cond_stmt.s2, indent + 1);
                }
                break;
            case END_DECL:
                printf("ENDDECL\n");
        ...
        case VARIABLE: {
            ...
            if (root->index) {
                ...
                if (index < 0 || index >= symbolTable[symbolIndex].array_size) {
                    fprintf(stderr, "Error: Array index out of bounds for '%s' on line %d\n", root->name, line_num);
                    exit(1);
                }
...
checking updateArrayElement(char* name, int index, int val) {
    ...
    if (symbolIndex == -1) {
        return UNDECL; // Should not happen, but good to check
    }
...
void freeStmtList(stmt_list* root) {
    while (root) {
        ...
        // Free the node-specific data based on its type
        switch (temp->type) {
            case ASSIGN:
            case WRITE_STMT:
